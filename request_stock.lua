...
--
---------------------------------------------------------------------------
-- Based on RSWarehouse.lua script by Scott Adkins <adkinss@gmail.com>
-- (Zucanthor):  Original file header below.
---------------------------------------------------------------------------
--
-- RSWarehouse.lua
...
-- Initialize Bridge
local bridge = peripheral.find("meBridge") or error("Bridge not found.")
...
function scanWorkRequests(mon, ctrlr, chest)
    ...
    for w in pairs(workRequests) do
		-- Some work requests are for virtual (like Feul), and thus have no items
		-- and no way for us to fill them (at this point, anyway).  It'd be nice
		-- to have a global table of virtual names that would allow lookup of what
		-- items fit that name, and then return them here.  TODO:
		-- BUGBUG:  This skips adding item to display, which it should do with a
		--          "Skipped" status.
		if #workRequests[w].items > 0 then
